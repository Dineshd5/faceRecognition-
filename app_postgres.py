...
# These will be set automatically by Render Environment Variables
DB_HOST = os.environ.get("DB_HOST", "localhost")
DB_NAME = os.environ.get("DB_NAME", "deepface")
DB_USER = os.environ.get("DB_USER", "postgres")
DB_PASS = os.environ.get("DB_PASS", "883842")
...
def blur_others(target_name, group_photo):
    ...
    try:
        ...
        for face_obj in results:
            ...
            padding_w = int(w * 0.2)
            padding_h = int(h * 0.2)
            y1, y2 = max(0, y - padding_h), min(original_img_rgb.shape[0], y + h + padding_h)
            x1, x2 = max(0, x - padding_w), min(original_img_rgb.shape[1], x + w + padding_w)
            ...
            if distance <= THRESHOLD:
                ...
                if roi.size > 0:
                    ...
                    original_img_rgb[y1:y2, x1:x2] = blended
